...
typedef enum {
    ...
    MTK_NFC_JNI_P2P_SNL_REQ,
    MTK_NFC_JNI_P2P_SNL_RES,
    #endif
    ...
    #if 1   //For AOSP HAL
    MTK_NFC_AOSP_HAL_START = 1000,
    MTK_NFC_AOSP_HAL_OEPN_REQ,
    MTK_NFC_AOSP_HAL_OEPN_RES,
    MTK_NFC_AOSP_HAL_CLOSE_REQ,
    MTK_NFC_AOSP_HAL_CLOSE_RES,
    MTK_NFC_AOSP_HAL_WRITE_REQ,
    MTK_NFC_AOSP_HAL_WRITE_RES,
    MTK_NFC_AOSP_HAL_CORE_INITED_REQ,
    MTK_NFC_AOSP_HAL_CORE_INITED_RES,
    MTK_NFC_AOSP_HAL_PRE_DISCOVERY_REQ,
    MTK_NFC_AOSP_HAL_PRE_DISCOVERY_RES,
    MTK_NFC_AOSP_HAL_CTRL_GRANT_NTF,
    MTK_NFC_AOSP_HAL_CTRL_GRANT_REQ,
    MTK_NFC_AOSP_HAL_CTRL_GRANT_RES,
    MTK_NFC_AOSP_HAL_PWR_CYCLE_REQ,
    MTK_NFC_AOSP_HAL_PWR_CYCLE_RES,
    
    MTK_NFC_AOSP_HAL_END,//case this is the last AOSP HAL msg , please add AOSP HAL msg before this.
    #endif  //For AOSP HAL
    
    MTK_NFC_META_GET_SELIST_REQ,
    MTK_NFC_META_GET_SELIST_RSP,
    
    MTK_NFC_JNI_FIELD_ONOFF_NTF,
    
    MTK_NFC_JNI_ACTIVATE_SE_INTERFACE_REQ,
    MTK_NFC_JNI_ACTIVATE_SE_INTERFACE_RSP,
...
#define MTK_NFC_AOSP_HAL_WRITE_DATA_MAX_LEN  (520)
...
#define IS_MSG_CAT_BELONGS_TO_AOSP_HAL(msgtype) (((msgtype)>= MTK_NFC_AOSP_HAL_START)&&((msgtype)<= MTK_NFC_AOSP_HAL_END))
...
#define NFC_SENSB_RES_MAX_LEN 20//for China 2G ID Card issue
//12//
#define NFC_NFCID0_MAX_LEN 8//for China 2G ID Card issue
//4
...
typedef struct mtk_nfc_fieldinfo_ntf_s
{
    UINT8 RFFieldStatus;  //b0 = 1 , field on , b0 = 0 , field off
}mtk_nfc_fieldinfo_ntf_t;

typedef struct mtk_nfc_activate_se_interface_req
{
    UINT8 u1NfceeId;
}mtk_nfc_activate_se_interface_req_t;

typedef struct mtk_nfc_activate_se_interface_rsp
{
    UINT32 u4Result; // 0 success, 1 fail, 2 invalid param
}mtk_nfc_activate_se_interface_rsp_t;

/*************Hal AOSP Msg  start**********************/
typedef struct mtk_nfc_halaosp_oepn_res
{
    UINT32 u4Result; // 0 success, 1 fail, 2 invalid param
}mtk_nfc_aosp_hal_oepn_res_t;

typedef struct mtk_nfc_halaosp_close_res
{
    UINT32 u4Result; // 0 success, 1 fail, 2 invalid param
}mtk_nfc_aosp_hal_close_res_t;

///AOSP write
typedef struct mtk_nfc_aosp_hal_write_req
{
    UINT32 len;
    UINT8 data[MTK_NFC_AOSP_HAL_WRITE_DATA_MAX_LEN];
}mtk_nfc_aosp_hal_write_req_t;

typedef struct mtk_nfc_aosp_hal_write_res
{
    UINT32 len;
    UINT8 data[MTK_NFC_AOSP_HAL_WRITE_DATA_MAX_LEN];
}mtk_nfc_aosp_hal_write_res_t;

typedef struct mtk_nfc_halaosp_power_cycle_res
{
    UINT32 u4Result; // 0 success, 1 fail, 2 invalid param

}mtk_nfc_halaosp_power_cycle_res_t;

typedef struct mtk_nfc_halaosp_core_inited_res
{
    UINT32 u4Result; // 0 success, 1 fail, 2 invalid param
}mtk_nfc_halaosp_core_inited_res_t;

typedef struct mtk_nfc_halaosp_ctrl_grant_res
{
    UINT32 u4Result; // 0 success, 1 fail, 2 invalid param
}mtk_nfc_halaosp_ctrl_grant_res_t;

typedef struct mtk_nfc_halaosp_ctrl_grant_ntf
{
    UINT32 u4Result; // 0 success, 1 fail, 2 invalid param
}mtk_nfc_halaosp_ctrl_grant_ntf_t;
typedef struct mtk_nfc_halaosp_pre_discovery_res
{
    UINT32 u4Result; // 0 success, 1 fail, 2 invalid param
}mtk_nfc_halaosp_pre_discovery_res_t;
/*************Hal AOSP Msg  end**********************/
