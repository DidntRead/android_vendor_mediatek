...
//         ret = MTK_NFC_ERROR;
//    }
    NFCD("%s:g_nfc_event_cond[%d]:%x",__FUNCTION__,event_idx);
...
//    if (pthread_cond_wait(&g_nfc_event_cond[event_idx], &g_nfc_event_mtx[event_idx]))
    NFCD("%s:g_nfc_event_cond[%d]:%x",__FUNCTION__,event_idx,g_nfc_event_cond[event_idx]);
    NFCD("%s:g_hMutex[%d]:%x",__FUNCTION__,mutex_idx,g_hMutex[event_idx]);
