...
static const char *g_pcNVM            = "/nvdata";
static const char *g_pcNVM_BT         = "/nvdata/APCFG/APRDEB/BT_Addr";
static const char *g_pcNVM_AllFile    = "/nvdata/AllFile";
static const char *g_pcNVM_AllMap     = "/nvdata/AllMap";
static const char *g_pcNVM_Flag       =
    "/nvdata/RestoreFlag";//The File Will be Created after restore
static const char *g_pcNVM_BackFlag   =
    "/nvdata/BackupFlag";//The file will be updated after modifying important data

static const char *g_pcNVM_APCalFile  = "/nvdata/APCFG/APRDCL";
static const char *g_pcNVM_APRdebFile = "/nvdata/APCFG/APRDEB";
static const char *g_pcNVM_MD         = "/nvdata/md/NVRAM";
static const char *g_pcNVM_MDCalFile  = "/nvdata/md/NVRAM/CALIBRAT";
static const char *g_pcNVM_MDRdebFile = "/nvdata/md/NVRAM/NVD_IMEI";
static const char *g_pcNVM_MDImptFile = "/nvdata/md/NVRAM/IMPORTNT";
static const char *g_pcNVM_MDDataFile = "/nvdata/md/NVRAM/NVD_DATA";
static const char *g_pcNVM_MDCoreFile = "/nvdata/md/NVRAM/NVD_CORE";
//Add for second modem for MT658*
static const char *g_pcNVM_MD2CalFile  = "/nvdata/md2/NVRAM/CALIBRAT";
static const char *g_pcNVM_MD2RdebFile = "/nvdata/md2/NVRAM/NVD_IMEI";
static const char *g_pcNVM_MD2ImptFile = "/nvdata/md2/NVRAM/IMPORTNT";
//End of Comment

/*Add for C2K modem*/
static const char *g_pcNVM_MD3CalFile  = "/nvdata/md3/NVRAM/CALIBRAT";
static const char *g_pcNVM_MD3RdebFile = "/nvdata/md3/NVRAM/NVD_IMEI";
static const char *g_pcNVM_MD3ImptFile = "/nvdata/md3/NVRAM/IMPORTNT";

//LTE support
static const char *g_pcNVM_MD5CalFile  = "/nvdata/md5/NVRAM/CALIBRAT";
static const char *g_pcNVM_MD5RdebFile = "/nvdata/md5/NVRAM/NVD_IMEI";
static const char *g_pcNVM_MD5ImptFile = "/nvdata/md5/NVRAM/IMPORTNT";
static const char *g_pcNVM_Via = "/nvdata/md_via";
...
#define FSTAB_PREFIX "/nvdata/fstab."
...
int NVM_Init(void) {
	int ret_callback = 0;
	int i = 0;
	NVRAM_PLATFORM_T nvram_platform = {0};
	int fd = 0;
	int error = 0;
	int ret;
...
static bool NVM_SetCheckSum(unsigned int ulCheckSum) {
	int iFileDesc_map = 0;
	int iResult;
	int iLength = sizeof(unsigned int);
...
bool NVM_RestoreFromFiles_OneFile(int eBackupType, int file_lid,
	...
	while (iFileNum > 0  && iFileNum < MAX_FILENUM) {
		...
		if ((filename == NULL) && (file_lid >= 0) && (pCfgFielTable != NULL)) {
			...
			if (strcmp(FileInfo->cFileName + 11, filename + 7) != 0) { //compare string after /vendor/nvdata and /nvdata
				--iFileNum;
				continue;
			}
...
bool NVM_CheckData_OneFile(int file_lid, const char * filename) {
	...
	NVBAK_LOG("NVM_CheckData_OneFile:%lx,%lx", ulSavedCheckSum, ulCheckSum);
...
Solve_Special_Lid:
	...
	if (g_new_nvram_lid[index_in_list].start_address % info.erasesize != 0
	        ...
	        g_new_nvram_lid[index_in_list].size > info.size) {
		NVRAM_LOG("Lid info in special info is error!!!\n");
		return FileID;
	}

	if ((!nvram_emmc_support()) && (!nvram_ufs_support())) {
		if ((pRecSize != NULL) && (*pRecSize < info.writesize)) {
	...
	if (IsRead) {
		...
		if ((!nvram_emmc_support()) && (!nvram_ufs_support())) {
			...
			if (g_new_nvram_lid[index_in_list].size < info.erasesize)
...
bool NVM_MiscLog(unsigned int level, const char *func, unsigned int line,
	...
	iResult = lseek(fd, start_address + offset, SEEK_SET);
	if (iResult  < 0) {
		NVRAM_LOG("NVM_MiscLog : seek log device fail start_address=%x,offset=%x!!!\n",
...
bool Check_UpdateStatus(void) {
	...
	if (-1 == val) {
		...
		if ((info.st_gid != AID_SYSTEM) || (ret >= 0 && statbuf.st_uid != AID_RADIO
			...
			if (chown("/nvdata", AID_ROOT, AID_SYSTEM)) {
		...
		if (vre >= 0 && stb.st_gid != AID_BLUETOOTH) {
			...
			if (chown("/nvdata/APCFG/APRDEB/BT_Addr", AID_ROOT, AID_BLUETOOTH)) {
				NVRAM_LOG("Fail to change BT group permission when upgrade\n");
				return false;
			}
			if (chmod("/nvdata/APCFG", 0771) ||
				chmod("/nvdata/APCFG/APRDEB", 0771) ||
				chmod("/nvdata/APCFG/APRDCL", 0771) ||
				chmod("/nvdata/APCFG/APRDCL/FILE_VER", 0664)) {
				NVRAM_LOG("Fail to change BT folder permission when upgrade for /nvdata\n");
				return false;
			}
		}
