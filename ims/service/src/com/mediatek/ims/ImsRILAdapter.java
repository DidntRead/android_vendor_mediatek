...
public class ImsRILAdapter extends ImsBaseCommands implements ImsCommandsInterface {
    ...
    /// @}
    /// M: For simulating network initiated USSI @{
    private final BroadcastReceiver mBroadcastReceiver = new BroadcastReceiver() {
        public void onReceive(Context context, Intent intent) {
            String[] ret = intent.getStringArrayExtra("eiusd");
            if (mUSSIRegistrants != null) {
                mUSSIRegistrants.notifyRegistrants(new AsyncResult(null, ret, null));
                if (mUSSIRegistrants.size() == 0) {
                    mNetworkInitUSSIRegistrants.notifyRegistrants(new AsyncResult(null, ret, null));
                }
            }
        }
    };
    ...
    public ImsRILAdapter(Context context) {
        ...
        filter.addAction("ACTION_USSI_SIMULATE");
    ...
    */
    ...
    public void turnOnVolte(Message response) {
        RILRequest rr = RILRequest.obtain(RIL_REQUEST_SET_VOLTE_ENABLE, response);
    ...
    public void turnOffVolte(Message response) {
        RILRequest rr = RILRequest.obtain(RIL_REQUEST_SET_VOLTE_ENABLE, response);
    ...
    public void turnOnWfc(Message response) {
        RILRequest rr = RILRequest.obtain(RIL_REQUEST_SET_WFC_ENABLE, response);
    ...
    public void turnOffWfc(Message response) {
        RILRequest rr = RILRequest.obtain(RIL_REQUEST_SET_WFC_ENABLE, response);
    ...
    public void turnOnImsVoice(Message response) {
        RILRequest rr = RILRequest.obtain(RIL_REQUEST_SET_IMS_VOICE_ENABLE, response);
    ...
    public void turnOffImsVoice(Message response) {
        RILRequest rr = RILRequest.obtain(RIL_REQUEST_SET_IMS_VOICE_ENABLE, response);
    ...
    public void turnOnImsVideo(Message response) {
        RILRequest rr = RILRequest.obtain(RIL_REQUEST_SET_IMS_VIDEO_ENABLE, response);
    ...
    public void turnOffImsVideo(Message response) {
        RILRequest rr = RILRequest.obtain(RIL_REQUEST_SET_IMS_VIDEO_ENABLE, response);
    ...
    class ImsRILReceiver implements Runnable {
        ...
        run() {
            ...
            try { for (;; ) {
                ...
                synchronized (mDtmfReqQueue) {
                    int i;
                    riljLog("mDtmfReqQueue queue size before " + mDtmfReqQueue.size());
   ...
   processSolicited (Parcel p) {
       ...
       if (error == 0 || p.dataAvail() > 0) {
           ...
           /// @}
            /// M: USSI RIL request @{
            case RIL_REQUEST_SEND_USSI: ret = responseVoid(p); break;
            case RIL_REQUEST_CANCEL_USSI: ret = responseVoid(p); break;
            /// @}
            /// M: set Ims capaiblity to MD @{
            case RIL_REQUEST_SET_VOLTE_ENABLE: ret = responseVoid(p); break;
            case RIL_REQUEST_SET_WFC_ENABLE: ret = responseVoid(p); break;
            case RIL_REQUEST_SET_IMS_VOICE_ENABLE: ret = responseVoid(p); break;
            case RIL_REQUEST_SET_IMS_VIDEO_ENABLE: ret = responseVoid(p); break;
            /// @}
            /// M: IMS ViLTE feature. @{
    ...
    static String requestToString(int request) {
        switch(request) {
            ...
            case RIL_REQUEST_SET_IMS_VOICE_ENABLE:
                return "RIL_REQUEST_SET_IMS_VOICE_ENABLE";
            case RIL_REQUEST_SET_IMS_VIDEO_ENABLE:
                return "RIL_REQUEST_SET_IMS_VIDEO_ENABLE";
            /// @}
            /// M: IMS ViLTE feature. @{
    ...
    private void processUnsolicited(Parcel p) {
        Rlog.i(IMS_RILA_LOG_TAG, " IMS processUnsolicited !!");
                ...
                try { switch(response) {
                    ...
                    /// M: USSI URC @{
                    case RIL_UNSOL_ON_USSI: ret = responseStrings(p); break;
                    /// @}
                    default:
                ...
                switch(response) {
                    ...
                    case RIL_UNSOL_ON_USSI:
                        ...
                        if (mUSSIRegistrants != null) {
                            mUSSIRegistrants.notifyRegistrants(new AsyncResult(null, ret, null));
                            if (mUSSIRegistrants.size() == 0) {
                                mNetworkInitUSSIRegistrants.notifyRegistrants(
                                    new AsyncResult(null, ret, null));
                            }
                        }
                        break;
    ...
    */
    ...
    /// M: Send/cancel USSI RIL request @{
    public void sendUSSI(int action, String ussiString, Message result) {
        Rlog.d(IMS_RILA_LOG_TAG, "sendUSSI: action=" + action + ", ussiString=" + ussiString);
        if (SystemProperties.get("persist.ims.ussi.ap").equals("1")) {
            Rlog.d(IMS_RILA_LOG_TAG, "Wrap sendUSSI");
            result.sendToTarget();
            return;
        }
        RILRequest rr = RILRequest.obtain(RIL_REQUEST_SEND_USSI, result);

        rr.mParcel.writeInt(2);
        rr.mParcel.writeString(Integer.toString(action));
        rr.mParcel.writeString(ussiString);

        if (IMS_RILA_LOGD) {
            riljLog(rr.serialString() + "> " + requestToString(rr.mRequest)
                + " action=" + action + ", ussiString=" + ussiString);
        }
        send(rr);
    }
    public void cancelPendingUssi(Message result) {
        Rlog.d(IMS_RILA_LOG_TAG, "cancelPendingUssi");
        if (SystemProperties.get("persist.ims.ussi.ap").equals("1")) {
            Rlog.d(IMS_RILA_LOG_TAG, "Wrap cancelPendingUssi");
            result.sendToTarget();
            return;
        }
        RILRequest rr
                = RILRequest.obtain(RIL_REQUEST_CANCEL_USSI, result);
    ...
    *
    * @param confCallId IMS(VoLTE) conference call id
    * @param callId The added participant call id
    * @param response Command response.
    *
    */
    public void inviteParticipantsByCallId(int confCallId, String callId, Message response) {
        if (getCallInfo(callId) == null) {
            Rlog.d(IMS_RILA_LOG_TAG, "Invite participants failed, call info is null");
            return;
        }
        int id = -1;
        try {
            ...
            Rlog.d(IMS_RILA_LOG_TAG, "Invite participants failed: id is not integer: " + callId);
            return;
        }
        addConferenceMember(confCallId, getCallInfo(callId).mCallNum, id, response);
    }
    /// @}
}
