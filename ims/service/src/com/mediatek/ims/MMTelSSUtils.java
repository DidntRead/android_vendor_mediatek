...
public class MMTelSSUtils {
    ...
    private static final List<String> OP19_MCCMNC_LIST = Arrays.asList("50501",
            "50511", "50571", "50572");

    // CSL
    private static final List<String> OP100_MCCMNC_LIST = Arrays.asList("45400",
            "45402", "45410", "45418");

    // PCCW
    private static final List<String> OP101_MCCMNC_LIST = Arrays.asList("45416",
            "45419", "45420", "45429");

    // SmarTone
    private static final List<String> OP102_MCCMNC_LIST = Arrays.asList("45406",
            "45415", "45417", "45500", "45506");

    // SingTel
    private static final List<String> OP103_MCCMNC_LIST = Arrays.asList("52501",
            "52502", "52507");

    // StarHub
    private static final List<String> OP104_MCCMNC_LIST = Arrays.asList("52505",
            "52506");

    // 3HK
    private static final List<String> OP106_MCCMNC_LIST = Arrays.asList("45403",
            "45404", "45405");

    // Taiwan Mobile
    private static final List<String> OP108_MCCMNC_LIST = Arrays.asList("46697");

    // Chunghwa
    private static final List<String> OP109_MCCMNC_LIST = Arrays.asList("46611",
            "46692");

    // CMHK (China mobile hongkong)
    private static final List<String> OP_CMHK_MCCMNC_LIST = Arrays.asList("45412",
            "45413");
    ...
    public static String getXcapRootUri(int phoneId) {
        ...
        } else if (isOp19IccCard(phoneId)) {
            simSrv.setUseHttpProtocolScheme(true);
            simSrv.setElementUpdateContentType(true, null);
            simSrv.setHandleError409(false);
            simSrv.setFillCompleteForwardTo(false);
            simSrv.setXcapNSPrefixSS(true);
        } else if (isOp102IccCard(phoneId)) {
            simSrv.setUseHttpProtocolScheme(true);
            simSrv.setElementUpdateContentType(true, null);
            simSrv.setHandleError409(false);
            simSrv.setFillCompleteForwardTo(false);
            simSrv.setXcapNSPrefixSS(false);
        } else if (isOp103IccCard(phoneId)) {
            simSrv.setUseHttpProtocolScheme(true);
            simSrv.setElementUpdateContentType(true, null);
            simSrv.setHandleError409(false);
            simSrv.setFillCompleteForwardTo(false);
            simSrv.setXcapNSPrefixSS(false);
        } else if (isOp104IccCard(phoneId)) {
            simSrv.setUseHttpProtocolScheme(true);
            simSrv.setElementUpdateContentType(true, null);
            simSrv.setHandleError409(false);
            simSrv.setFillCompleteForwardTo(false);
            simSrv.setXcapNSPrefixSS(false);
        } else if (isOp108IccCard(phoneId)) {
            simSrv.setUseHttpProtocolScheme(true);
            simSrv.setElementUpdateContentType(true, null);
            simSrv.setHandleError409(false);
            simSrv.setFillCompleteForwardTo(false);
    ...
    public static boolean isOp19IccCard(int phoneId) {
        if (SystemProperties.get("ro.mtk_bsp_package").equals("1")) {
            return false;
        }
        ...
        int subId = SubscriptionManager.getSubIdUsingPhoneId(phoneId);
        String mccMnc = TelephonyManager.getDefault().getSimOperator(subId);
        Rlog.d(LOG_TAG, "isOp19IccCard(): mccMnc is " + mccMnc);
        return OP19_MCCMNC_LIST.contains(mccMnc);
    }

    public static boolean isOp100IccCard(int phoneId) {
        if (SystemProperties.get("ro.mtk_bsp_package").equals("1")) {
            return false;
        }
        ...
        int subId = SubscriptionManager.getSubIdUsingPhoneId(phoneId);
        String mccMnc = TelephonyManager.getDefault().getSimOperator(subId);
        Rlog.d(LOG_TAG, "isOp100ccCard(): mccMnc is " + mccMnc);
        return OP100_MCCMNC_LIST.contains(mccMnc);
    }

    public static boolean isOp101IccCard(int phoneId) {
        if (SystemProperties.get("ro.mtk_bsp_package").equals("1")) {
            return false;
        }
        ...
        int subId = SubscriptionManager.getSubIdUsingPhoneId(phoneId);
        String mccMnc = TelephonyManager.getDefault().getSimOperator(subId);
        Rlog.d(LOG_TAG, "isOp101ccCard(): mccMnc is " + mccMnc);
        return OP101_MCCMNC_LIST.contains(mccMnc);
    }

    public static boolean isOp102IccCard(int phoneId) {
        if (SystemProperties.get("ro.mtk_bsp_package").equals("1")) {
            return false;
        }
        ...
        int subId = SubscriptionManager.getSubIdUsingPhoneId(phoneId);
        String mccMnc = TelephonyManager.getDefault().getSimOperator(subId);
        Rlog.d(LOG_TAG, "isOp102IccCard(): mccMnc is " + mccMnc);
        return OP102_MCCMNC_LIST.contains(mccMnc);
    }

    public static boolean isOp103IccCard(int phoneId) {
        if (SystemProperties.get("ro.mtk_bsp_package").equals("1")) {
            return false;
        }
        ...
        int subId = SubscriptionManager.getSubIdUsingPhoneId(phoneId);
        String mccMnc = TelephonyManager.getDefault().getSimOperator(subId);
        Rlog.d(LOG_TAG, "isOp103IccCard(): mccMnc is " + mccMnc);
        return OP103_MCCMNC_LIST.contains(mccMnc);
    }

    public static boolean isOp104IccCard(int phoneId) {
        if (SystemProperties.get("ro.mtk_bsp_package").equals("1")) {
            return false;
        }
        ...
        int subId = SubscriptionManager.getSubIdUsingPhoneId(phoneId);
        String mccMnc = TelephonyManager.getDefault().getSimOperator(subId);
        Rlog.d(LOG_TAG, "isOp104IccCard(): mccMnc is " + mccMnc);
        return OP104_MCCMNC_LIST.contains(mccMnc);
    }

    public static boolean isOp106IccCard(int phoneId) {
        if (SystemProperties.get("ro.mtk_bsp_package").equals("1")) {
            return false;
        }
        ...
        String mccMnc = TelephonyManager.getDefault().getSimOperator(subId);
        Rlog.d(LOG_TAG, "isOp104IccCard(): mccMnc is " + mccMnc);
        return OP106_MCCMNC_LIST.contains(mccMnc);
    }

    public static boolean isOp108IccCard(int phoneId) {
        if (SystemProperties.get("ro.mtk_bsp_package").equals("1")) {
            return false;
        }
        ...
        int subId = SubscriptionManager.getSubIdUsingPhoneId(phoneId);
        String mccMnc = TelephonyManager.getDefault().getSimOperator(subId);
        Rlog.d(LOG_TAG, "isOp108IccCard(): mccMnc is " + mccMnc);
        return OP108_MCCMNC_LIST.contains(mccMnc);
    }

    public static boolean isOp109IccCard(int phoneId) {
        if (SystemProperties.get("ro.mtk_bsp_package").equals("1")) {
            return false;
        }
        ...
        int subId = SubscriptionManager.getSubIdUsingPhoneId(phoneId);
        String mccMnc = TelephonyManager.getDefault().getSimOperator(subId);
        Rlog.d(LOG_TAG, "isOp109IccCard(): mccMnc is " + mccMnc);
        return OP109_MCCMNC_LIST.contains(mccMnc);
    }

    public static boolean isOpCMHKIccCard(int phoneId) {
        if (SystemProperties.get("ro.mtk_bsp_package").equals("1")) {
            return false;
        }
        ...
        int subId = SubscriptionManager.getSubIdUsingPhoneId(phoneId);
        String mccMnc = TelephonyManager.getDefault().getSimOperator(subId);
        Rlog.d(LOG_TAG, "isOpCMHKIccCard(): mccMnc is " + mccMnc);
        return OP_CMHK_MCCMNC_LIST.contains(mccMnc);
    }
        ...
        if ("http".equals(xcapRoot.substring(0, xcapRoot.lastIndexOf(':')))
            ...
            if (isOp18IccCard(phoneId)) {
                xcapRoot += ":7077";
                    ...
                    || isOp104IccCard(phoneId)
                    || isOp108IccCard(phoneId)) {
                xcapRoot += ":8090";
            } else if (isOp103IccCard(phoneId)) {
                xcapRoot += ":8080";
            }
