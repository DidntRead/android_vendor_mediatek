...
public class ImsCallSessionProxy extends IImsCallSession.Stub {
    ...
    /// M: ALPS02604117, keep the conference participants user-entity.
    /// key = parsed user-entity number, value = full user-entity uri. @{
    private LinkedHashMap mConfParticipantsUri = new LinkedHashMap<String, String>();
    /// @}
    ...
    /// M: For UE initial USSI request or response network USSI @{
    ...
    /// M: For return message of sending/cancel USSI @{
    ...
    public void close() {
        ...
        if (vtProvider != null) {
            try {
                vtProvider.setUIMode(ImsVTProviderUtil.UI_MODE_DESTROY);
                mVTProvider = null;
            } catch (RemoteException e) {
                Rlog.e(LOG_TAG, "RemoteException vtProvider.setUIMode()");
            }
        }
    }
    ...
    public void start(String callee, ImsCallProfile profile) {
        ...
        // ALPS02622596, OP08 requirement: use CLIR_SUPPRESSION to replace CLIR_DEFAULT.
        ...
        final List<String> OP08_MCCMNC_LIST = Arrays.asList("23203", "23204", "21901", "23001",
        "21630", "29702", "20416", "20420", "26002", "22004", "23430", "310160",
        "310260", "310490", "310580", "310660");
        if (OP08_MCCMNC_LIST.contains(mccMnc) && clirMode == CommandsInterface.CLIR_DEFAULT) {
            Rlog.d(LOG_TAG, "op08, condsider CLIR default as suppression.");
    ...
    public void merge() {
        ...
        int phoneId = RadioCapabilitySwitchUtil.getMainCapabilityPhoneId();
        String mccMnc = TelephonyManager.getDefault().getSimOperatorNumericForPhone(phoneId);
        final List<String> swapConfMccMncList
            = Arrays.asList(
                // 1. OP16:
                "23430", "23431", "23432", "23433", "23434");

        boolean needSwapConfToFg = swapConfMccMncList.contains(mccMnc);
        ...
        Rlog.d(LOG_TAG, "merge command- my call: conference type=" + myCallInfo.mIsConference +
                ...
                " needSwapConfToFg=" + needSwapConfToFg + " mccMnc=" + mccMnc);
        ...
        ///ALPS02709186, thread safe issue
        //It may be false when receive 3, 130, so move it here from the end.
        mIsMerging = true;
    ...
    public void removeParticipants(String[] participants) {
        ...
        mPendingParticipantStatistics = participants.length;
        if (mCallId != null || mPendingParticipantStatistics == 0) {
            /// M: ALPS02604117, get participant uri. @{
            String participantAddr = mPendingParticipantInfo[mPendingParticipantInfoIndex];
            String participantUri = (String)mConfParticipantsUri.get(participantAddr);
            if (participantUri == null || !participantUri.startsWith("sip:")) {
                participantUri = participantAddr;
            }
            /// @}
        ...
        private void handleImsConfCallMessage(int len, int callId, String data) {
            try {
                ...
                for (ConferenceCallMessageHandler.User u : users) {
                    ...
                    /// M: ALPS02604117, keep the user-entity uri. @{
                    String userEntity = u.getEntity();
                    String userAddr = getUserNameFromSipTelUriString(u.getEntity());
                    mConfParticipantsUri.put(userAddr, userEntity);
                    /// @}
                    confInfo.putString(ImsConferenceState.USER, userAddr);
            ...
            switch (msg.what) {
                ...
                case EVENT_GET_LAST_CALL_FAIL_CAUSE:
                    ...
                    if (ar.exception != null) {
                        ...
                        if (sipCauseCode == ImsReasonInfo.CODE_SIP_REDIRECTED_EMERGENCY) {
                            try {
    ...
    private void mergeFailed() {
       ...
       if (mConfSession instanceof ImsCallSessionProxy) {
           ...
           /// M: ALPS02588163, mConfSession should set to null, or it might be
           /// closed twice.
           mConfSession = null;
       }
    }
    ...
    private void updateCallStateForWifiOffload(int callState) {
        ...
        try {
            mWfoService.updateCallState(callId, callType, wosCallState);
        } catch (RemoteException e) {
